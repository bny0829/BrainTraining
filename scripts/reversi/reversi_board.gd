...
var last_move := -1
var hint_index := -1  # 「提示」按鈕建議的最佳落點（空心圈，跟合法手小點區分）
...
func _draw() -> void:
	...
	# 提示按鈕建議的最佳落點：空心圈
	if hint_index >= 0 and stones[hint_index] == ReversiLogic.EMPTY:
		draw_arc(_center_of(hint_index), s * 0.32, 0.0, TAU, 32, AppTheme.ACCENT, 4.0, true)
